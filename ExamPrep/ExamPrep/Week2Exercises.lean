...
def replicate (n : Nat) (x : α) : List α := sorry
...
def zipWith (f : α → β → γ) (xs : List α) (ys : List β) : List γ := sorry
...
def take (n : Nat) (xs : List α) : List α := sorry
def drop (n : Nat) (xs : List α) : List α := sorry
...
def last? (xs : List α) : Option α := sorry
...
def interleave (xs ys : List α) : List α := sorry
...
def chunk (n : Nat) (xs : List α) : List (List α) := sorry
...
def isPalindrome [BEq α] (xs : List α) : Bool := sorry
...
def rotate (n : Nat) (xs : List α) : List α := sorry
...
def windows (n : Nat) (xs : List α) : List (List α) := sorry
...
def flatten (xs : List (List α)) : List α := sorry
...
def unique [BEq α] (xs : List α) : List α := sorry
...
def maximum? (xs : List Nat) : Option Nat := sorry
def minimum? (xs : List Nat) : Option Nat := sorry
...
def partition (p : α → Bool) (xs : List α) : List α × List α := sorry
...
def insertSorted (x : Nat) (xs : List Nat) : List Nat := sorry
def insertionSort (xs : List Nat) : List Nat := sorry
...
partial def mergeSort (xs : List Nat) : List Nat := sorry
...
partial def quickSort (xs : List Nat) : List Nat := sorry
...
partial def selectionSort (xs : List Nat) : List Nat := sorry
...
def bubble (xs : List Nat) : List Nat := sorry
def bubbleSort (xs : List Nat) : List Nat := sorry
...
def sumTree (t : Tree Int) : Int := sorry
def mirror (t : Tree α) : Tree α := sorry
def contains [BEq α] (t : Tree α) (x : α) : Bool := sorry
...
def preOrder (t : Tree α) : List α := sorry
def inOrder (t : Tree α) : List α := sorry
def postOrder (t : Tree α) : List α := sorry
...
def mapTree (f : α → β) (t : Tree α) : Tree β := sorry
def foldTree (base : β) (f : α → β → β → β) (t : Tree α) : β := sorry
...
-- ============================================================
-- ### Exercise 23: BST insert and search
-- A BST satisfies: left subtree values < node value,
-- right subtree values > node value.
-- ============================================================

def bstInsert (t : Tree Nat) (x : Nat) : Tree Nat := sorry
def bstSearch (t : Tree Nat) (x : Nat) : Bool := sorry
...
def bstToList (t : Tree Nat) : List Nat := sorry
...
partial def collatzSeq (n : Nat) : List Nat := sorry
partial def longestCollatz (limit : Nat) : Nat := sorry
